...
class DepositeDebtScreen extends ConsumerStatefulWidget {
  const DepositeDebtScreen({super.key});

  @override
  ConsumerState<DepositeDebtScreen> createState() => _DepositeDebtScreenState();
}
...
class _DepositeDebtScreenState extends ConsumerState<DepositeDebtScreen> {
  @override
  void initState() {
  ...
  void doneBtn(
    ...
    if (paymentDate.isEmpty || price.isEmpty || description.isEmpty) {
      ...
      ref.read(buttonLoadingNotifierProvider.notifier).changeIndex(false);
    } else if (double.parse(debt.amount) <= double.parse(price)) {
      showDialog(
        context: context,
        builder: (context) {
          return AlertDialog(
            ...
            actions: <Widget>[
              ...
              TextButton(
                onPressed: () {
                  runPaid(debt, paymentDate, price, description, transH);
                },
                child: Text(
  ...
  void runPaid(Debt debt, paymentDate, price, String description,
      AppLocalizations transH) async {
    {
      String depositedDate =
          DateFormat('yyyy-MM-dd H:mm:ss').format(DateTime.now());
      var data =
          await ref.read(debtControllerProvider.notifier).depositedOnDebt(
                debt.id,
                paymentDate,
                price,
                description,
      ...
      if (data['response'] == 'Validation error') {
        ...
        Debt debtRecord = debt.copyWith(
          amount: (int.parse(debt.amount) - int.parse(price)).toString(),
