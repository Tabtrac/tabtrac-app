...
class UpdateLiabiltyScreen extends ConsumerStatefulWidget {
  const UpdateLiabiltyScreen({super.key});

  @override
  ConsumerState<UpdateLiabiltyScreen> createState() =>
      _UpdateLiabiltyScreenState();
}
...
class _UpdateLiabiltyScreenState extends ConsumerState<UpdateLiabiltyScreen> {
  @override
  void initState() {
  ...
  void fillData() async {
    ...
    ref.read(nameControllerProvider).text = liability.creditorName;
    ref.read(descriptionControllerProvider).text = liability.description;
    ref.read(priceControllerProvider).text = liability.amount;
    ref.read(paymentDateControllerProvider).text = liability.paymentDate;
    currencies = liability.currency;
    liability.phoneNumber != null
        ? ref.read(phoneNumberControllerProvider).text =
            liability.phoneNumber.toString()
        : null;
    liability.email != null
        ? ref.read(emailControllerProvider).text = liability.email.toString()
        : null;
  }
  ...
  Widget build(BuildContext context) {
    ...
    final nCurrency = ref.watch(currencyNotifierProvider);
    return Scaffold(
      appBar: AppBar(
        ...
        centerTitle: true,
        title: Text(
          transH.editLiabity.toUpperCase(),
          style: TextStyle(
            color: isDarkMode
                ? AppColors.darkThemeColor
                : AppColors.lightThemeColor,
  ...
  void doneBtn(
    ...
    if (debtorName.isEmpty ||
      ...
      if (await utilityController.isConnected()) {
        ...
        if (data['response'] == 'Validation error') {
        ...
        } else if (data['response'] == "Liability Updated Successfully") {
          Liability liabilityRecord = Liability(
            id: liability.id,
          ...
          Navigator.pop(context);
          AnimatedSnackBar.material(
            "${transH.success}, ${transH.liabilityUpdated}".capitalizeFirst.toString(),
        ...
        Liability updatedData = Liability(
          id: liability.id,
          creditorName: debtorName,
          status: liability.status,
          currency: currency,
        ...
        ref.read(liabilityNotifierProvider.notifier).changeValues(updatedData);
        if (liability.offlineData != null && liability.offlineData == true) {
          await ref
              .read(liabilityControllerControllerProvider.notifier)
              .editOfflineData(
                context,
